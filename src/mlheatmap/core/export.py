...
def _write_results_workbook(writer, session, include_normalized_expression: bool) -> None:
    """Write the current analysis results and metadata to an Excel workbook."""
    import pandas as pd
    ...
    if include_normalized_expression and session.normalized is not None:
        df = pd.DataFrame(
            session.normalized,
...
def export_results_excel(session, include_normalized_expression: bool = False) -> bytes:
    """Export analysis results to an Excel workbook.

    Compact export excludes the full normalized matrix by default to keep
    workbook size and export time manageable for large RNA-seq datasets.
    """
    import pandas as pd

    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        _write_results_workbook(writer, session, include_normalized_expression)
